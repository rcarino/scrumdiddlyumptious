...
This folder contains a bunch of scripts that the maintainers of the scrumdiddlyumptious tutorial project
...
- **snapshot-web.sh**: Run this script to iterate through the step tags checking out the application
  code and copying it to a folder `angular-scrumdiddlyumptious-snapshots-web`, which can then be used to update
  the gh-pages branch.

## Sandbox and Testing

Similar to how we can generate the live demo pages, We can also snapshot all the steps into a
folder `angular-scrumdiddlyumptious-snapshots`, which then contains an additional folder called `sandbox`.
...
- **snaphot.sh**: Run this script to generate the `angular-scrumdiddlyumptious-snapshots` folder, containing
